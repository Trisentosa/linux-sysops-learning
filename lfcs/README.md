...
- Everything important happening in linux is saved in log files
- Logging Daemons: application that job is to collect and store log messages from different sources
  - `rsyslog` (rocket fast system for log processing): stored system log files in `/var/log` directory. note most of these files required root access
    - `/var/log/syslog`: main system log file, contains all system log messages
    - `/var/log/auth.log`: authentication log file, contains all authentication log messages
    - `/var/log/kern.log`: kernel log file, contains all kernel log messages
    - `/var/log/daemon.log`: daemon log file, contains all daemon log messages
    - `/var/log/messages`: general log file, contains all log messages
  - You may notice file name like `auth.log.1`, `auth.log.2.gz`, etc. This is because `rsyslog` will rotate the log file when it reach a certain size. The number represent the rotation count (smaller number mean more recent), and the `.gz` represent the file is compressed using gzip
  - To have a live view of log file using tail and follow option
    - `tail -F /var/log/auth.log`
    - can combine it with `grep`, `less`, etc
- `journalctl`: lets us collect logs generated by specific command. will open file in `less`-like mode
  - Example:
    - `journalctl /usr/bin/sudo`: show all logs generated by sudo command
    - `journalctl -u sshd`: show all logs generated by sshd service
    - `journalctl -b <boot>`: show all logs generated since last boot (current boot = 0, previous boot = 1, etc)
    - `journalctl -f`: show all logs in **real time**
    - `journalctl -k`: show all kernel logs
    - `journalctl -u sshd -f`: show all logs generated by sshd service in real time
    - `journalctl -e`: directly go to the end of the log file
    - `journalctl -p <priority>`: show all logs with certain priority. commonly combined with `-b` to see all logs since last boot with certain priority
      - priority: 0 (emerg), 1 (alert), 2 (crit), 3 (err), 4 (warning), 5 (notice), 6 (info), 7 (debug) 
      - Example: `journalctl -b -p err`: show all logs since last boot with priority error or higher (e.g. error, critical, alert, emerg)
    - `journalctl -g <pattern>`: show all logs that match certain pattern (regex supported)
      - Example: `journalctl -g "Failed password"`: show all logs that contain "Failed password"
